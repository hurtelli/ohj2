        ...
        if(command == "U" or command == "u") {
            ...
            if(student_numbers.find(parts.at(1)) != student_numbers.end()){
                ...
                if(!is_valid_phone_number(nsnumber)){
                    ...
                    const auto& sid = student_numbers.at(parts.at(1))->user_id;
                    student_numbers.at(parts.at(1))->phone_number = nsnumber;
                    user_ids.at(sid)->phone_number = nsnumber;

                    std::ofstream sdata(file_name);
                    for(const auto& s : user_ids){
                        sdata << s.second->student_number<<";"<<s.second->user_id<< ";"<<s.second->name<<";"<<s.second->phone_number<<";"<<s.second->email<<";"<<s.second->skype<<std::endl;
                    }
                    sdata.close();
                }
            }else{
